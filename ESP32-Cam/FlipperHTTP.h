...
#ifndef FLIPPER_HTTP_CAM_H
#define FLIPPER_HTTP_CAM_H
...
#define LED_PIN 4 // thanks Derek Jamison
...
public:
    ...
    bool isConnectedToWifi() { return WiFi.status() == WL_CONNECTED; }                                                         // Check if the ESP32-Cam is connected to Wifi
    void ledAction(int pin = LED_PIN, int timeout = 250);                                                                      // Turn on and off the LED
    void ledStart();                                                                                                           // Display LED sequence when ESP32-Cam is first connected to the Flipper
    void ledStatus();                                                                                                          // Starting LED (Green only)
    void ledOff() { digitalWrite(LED_PIN, OFF); }                                                                              // Turn off LED
    ...
    bool readSerialSettings(String receivedData, bool connectAfterSave);                                                       // Read serial data and save settings
    bool saveWifiSettings(String data);                                                                                        // Save Wifi settings to SPIFFS
